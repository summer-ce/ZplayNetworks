...
using ZplaySDKEditor.ZplaySDK.Editor;

public class ReporterEditor : Editor
{
	[MenuItem("Zplay/Reporter/Create",false,0)]
	public static void CreateReporter()
	{
        if (GameObject.Find("Reporter"))
        {
            Debug.LogError("Reporter gameobject already exists please do not create the Reporter gameobject repeatedly");
            return;
        }
    ...
    private const string macroName = "RELEASE_STORE";
    [MenuItem("Zplay/Reporter/Store", false, 1)]
    public static void ReporterStore()
    {
        MacroDefinitionControler.SettingReporterSymbolsFlag(macroName, false);
    }
    [MenuItem("Zplay/Reporter/StoreClear", false, 2)]
    public static void ReporterStorteClear()
    {
        MacroDefinitionControler.SettingReporterSymbolsFlag(macroName,true);
    }
}
